...
这是 Android 版 Release 0.1.1。安装后，支持角色发动“通常攻击 1”和“通常攻击 2”时出现的角色演出都会替换为该角色的高画质成人变身动画（R18），战斗和非战斗动画预览均支持。实际攻击动作、伤害和战斗速度不会改变。
...
已经使用 Android 版 0.1 的用户无需卸载：下载并完整解压 0.1.1，直接重新运行新的 BAT 即可。手机上仍然有效的缓存和已下载资源会继续复用。
